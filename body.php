    ...
    <div class="col-sm-3">
        <form id="form" action="index.php" method="post">
            <div style="display: none" class="div" id="div1">
                ...
                Фамилия: <input style="<?php checkColor('last_name') ?>" class="form-control" name="last_name" value="<?= $_POST['last_name'] ?>" type="text">
                <span style="color: red">*</span>
                Имя: <input style="<?php checkColor('first_name') ?>" class="form-control" name="first_name" value="<?= $_POST['first_name'] ?>" type="text">
                Отчество: <input class="form-control" name="patro" value="<?= $_POST['patro'] ?>" type="text">
                <span style="color: red">*</span>
                Пол: <select class="form-control" name="gender">
                    <?php
                    $ops = ['male' => 'Мужской', 'female' => 'Женский'];
                ...
                <span style="color: red">*</span>
                Дата рождения: <input style="<?php checkColor('birth_date') ?>" class="form-control" name="birth_date" value="<?= $_POST['birth_date'] ?>"
                                      type="date">
                <span style="color: red">*</span>
                Место рождения (по паспорту):
                <textarea style="<?php checkColor('birth_place') ?>" name="birth_place" class="form-control"><?= $_POST['birth_place'] ?></textarea>
                <span style="color: red">*</span>
                Гражданство: <select id="nation_select1" onclick="checkNation(1)" class="form-control" name="nation">
                    <?php
                    $ops = ['rus' => 'Российская Федерация', 'other' => 'Другая страна'];
                    foreach ($ops as $val => $name) {
                        echo '<option value="' . $val . '"' . ($_POST['nation'] == $val ? "selected='selected'" : '') . '>' . $name . '</option>';
                    }
                    ?>
                </select>
            ...
            <div style="display: none" class="div" id="div2">
                ...
                Вид документа: <select class="form-control" name="doc_type">
                    <?php
                    $ops = ['rus' => 'Паспорт гражданина РФ', 'other' => 'Паспорт гражданина иностранного государства'];
                    foreach ($ops as $val => $name) {
                ...
                <span style="color: red">*</span>
                Серия: <input style="<?php checkColor('doc_serial') ?>" class="form-control" name="doc_serial" value="<?= $_POST['doc_serial'] ?>" type="text">
                <span style="color: red">*</span>
                Номер: <input style="<?php checkColor('doc_num') ?>" class="form-control" name="doc_num" value="<?= $_POST['doc_num'] ?>" type="text">
                <span style="color: red">*</span>
                Дата выдачи: <input style="<?php checkColor('issue_date') ?>" class="form-control" name="issue_date" value="<?= $_POST['issue_date'] ?>"
                                    type="date">
                <span style="color: red">*</span>
                Орган, выдавший документ (как в паспорте):
                <textarea style="<?php checkColor('doc_giver') ?>" name="doc_giver" class="form-control"><?= $_POST['doc_giver'] ?></textarea>
            </div>
            <div style="display: none" class="div" id="div3">
                ...
                Страна: <select id="nation_select2" onclick="checkNation(2)" class="form-control" name="doc_nation">
                    ...
                    $ops = ['rus' => 'Российская Федерация', 'other' => 'Другая страна'];
                    foreach ($ops as $val => $name) {
                        echo '<option value="' . $val . '"' . ($_POST['doc_nation'] == $val ? "selected='selected'" : '') . '>' . $name . '</option>';
                    }
                    ?>
                </select>
                ...
                Страна: <select id="nation_select3" onclick="checkNation(3)" class="form-control" name="f_doc_nation">
                    ...
                    $ops = ['rus' => 'Российская Федерация', 'other' => 'Другая страна'];
                    foreach ($ops as $val => $name) {
                        echo '<option value="' . $val . '"' . ($_POST['f_doc_nation'] == $val ? "selected='selected'" : '') . '>' . $name . '</option>';
                    }
                    ?>
                </select>
            ...
            <div style="display: none" class="div" id="div4">
                ...
                Тип учебного заведения: <select class="form-control" name="edu_place_type">
                    <?php
                    $ops = ['school' => 'Школа', 'gymnasium' => 'Гимназия', 'lyceum' => 'Лицей', 'ptu' => 'Техникум',
                        'university' => 'Высшее учебное учереждение', 'other' => 'Другое'];
                    foreach ($ops as $val => $name) {
                ...
                Страна: <select id="nation_select4" onclick="checkNation(4)" class="form-control" name="edu_nation">
                    ...
                    $ops = ['rus' => 'Российская Федерация', 'other' => 'Другая страна'];
                    foreach ($ops as $val => $name) {
                        echo '<option value="' . $val . '"' . ($_POST['edu_nation'] == $val ? "selected='selected'" : '') . '>' . $name . '</option>';
                    }
                    ?>
                </select>
                ...
                <span style="color: red">*</span>
                Населённый пункт: <input style="<?php checkColor('edu_city') ?>" class="form-control" name="edu_city" value="<?= $_POST['edu_city'] ?>"
                                         type="text">
                <span style="color: red">*</span>
                Серия: <input style="<?php checkColor('edu_serial') ?>" class="form-control" name="edu_serial" value="<?= $_POST['edu_serial'] ?>" type="text">
                <span style="color: red">*</span>
                Название учебного заведения:
                <textarea style="<?php checkColor('edu_place_name') ?>" name="edu_place_name" class="form-control"><?= $_POST['edu_place_name'] ?></textarea>
                ...
                Язык: <select class="form-control" name="edu_lang">
                    <?php
                    $ops = ['en' => 'Английский', 'fr' => 'Французский', 'de' => 'Немецкий'];
                    foreach ($ops as $val => $name) {
                        echo '<option value="' . $val . '"' . ($_POST['edu_lang'] == $val ? "selected='selected'" : '') . '>' . $name . '</option>';
                    }
                    ?>
                </select>
            </div>
